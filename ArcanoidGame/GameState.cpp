...
namespace SnakeGame
{
	...
	void GameState::HandleWindowEvent(sf::Event& event)
	{
		...
		case GameStateType::MainMenu:
		{
			((GameStateMainMenuData*)data)->HandleGameState(event);
			break;
		}
		case GameStateType::Playing:
		{
			((GameStatePlayingData*)data)->HandleGameState(event);
			break;
		}
		case GameStateType::GameOver:
		{
			((GameStateGameOverData*)data)->HandleGameState(event);
			break;
		}
		case GameStateType::ExitDialog:
		{
			((GameStatePauseMenuData*)data)->HandleGameState(event);
			break;
		}
		case GameStateType::Records:
		{
			((GameStateRecordsData*)data)->HandleGameState(event);
			break;
		}
		default:
			assert(false); // We want to know if we forgot to implement new game state
